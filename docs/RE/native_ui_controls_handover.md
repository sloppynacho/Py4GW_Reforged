...
## 0. 2026-07-01 SESSION â€” definitive findings & the path forward (READ BEFORE CODING)

**The definitive answer (high confidence, from a 7-window WASM-first comparison â€” full spec in
...
`IUi::UiCtlDlgProc` flags `0x59`, teardown via msg `0xb â†’ IUi::PopCloser â†’ recursive FrameDestroy`, freeing
...
- âœ… **button** works everywhere (simple self-contained `CtlBtnProc`). âœ… **progress**, âœ… **group header** work.
- âœ… **checkbox glyph** fixed: flags `0x48300`â†’`0x18000` (the `0x40000` bit was forcing the button face). It now
  renders the correct texture and toggles **as a plain direct child of a normal window** â€” its ONLY remaining
  failure there is the native `[X]` teardown crash.
- âœ… **hyperlink click** fixed in `GWUI.py` (unique nonzero per-row child codes).
- âŒ Complex controls in every AD-HOC host tried (all crash/blank): `UiCtlContentPageProc` content page
  (crashes â€” half-initialized standalone); plain-container band member `FUN_0051d8e0` (hides children +
  ...
  controls â€” only the full instanced-dialog framework does.**
...
**RECOMMENDED NEXT STEP â€” build the host as ONE unit, not incrementally.** Every partial piece crashes because
each layer needs the others already present. Implement the whole **instanced content-dialog host** in one pass
against the 9-step spec in `native_dialog_layout_process.md` Â§"Reliable pattern": author the dispatcher to
...
**Isolated test scaffold:** `UI_RE/native_layout_test.py` (widget "Native Layout Test") â€” SEPARATE from the
...
after; **rename EXE functions** as you identify them (`rename_function_by_address`, ignore style warnings) â€”
a stream of `FUN_xxxx` means you're doing it backwards. **No bypasses** â€” the native flow (incl. the `[X]`)
...
text-button, edit box, progress bar, tabs, slider, group header â€” created in-client, styled with the real
GW textures, interactive via the engine's own state, and destroyable without crashing.

Code lives across two repos:
- **C++**: `C:\Users\Apo\Py4GW` â€” `include/py_ui.h` (creator functions), `src/py_ui.cpp` (pybind bindings).
- **Python**: `C:\Users\Apo\Py4GW_python_files` â€” `Py4GWCoreLib/GWUI.py` (high-level API),
  `stubs/PyUIManager.pyi` (type stubs), `UI_RE/gwui_controls_test.py` (in-client test harness).

**Build**: `cd C:\Users\Apo\Py4GW && cmake --build build --config RelWithDebInfo` â†’ copy
...
|---|---|---|
| **button** | âœ… WORKS | textured, clickable â€” simple `CtlBtnProc`; hosts fine as a frame-list item AND as a direct child on the dialog-base window |
| **tabs** | âœ… WORKS | real tab texture + switching (styled `UiCtlPageProc` 0x00885590 as PRIMARY proc) |
| **group section** | âœ… WORKS | header + collapsible member rows (app-side collapse via ShowItem msg 0x67) |
| **progress** | âœ… renders + animates | value-cycle works; close crash shared with the complex-control teardown (Â§0) |
| **checkbox** | âœ… glyph fixed / âŒ close crash | flags `0x48300`â†’`0x18000` (the `0x40000` bit was forcing the button face). Renders CORRECT texture + toggles as a plain direct child; ONLY failure is the native `[X]` teardown crash. Crashes/blanks in every non-direct-child host tried (Â§0) |
| **hyperlink** | âœ… click fixed / âŒ z-order | click now detected (unique nonzero per-row child codes in `GWUI.py`); still z-fights/buried as a selectable list â€” needs the instanced-dialog host |
| **slider** | âš ï¸ drags / âŒ wide+blank | too wide (rangeâ†’pixel; needs the `CCtlLayout` clamp); blank without the layout pass |
| **edit** | âŒ blank | needs BOTH the sibling `UiCtlBorderProc` box AND the layout pass â€” i.e. the instanced-dialog host (or ConfirmDialog nested `UiCtlDlg` child) |
| **radio** | âŒ generic/z-order | selectable-list rows render generic + buried; needs the host/layout |
| **native [X] close** | âŒ the dominant blocker | closing a window that hosts a complex control crashes (dangling hover hot-item); the native fix is `PopCloser` on a real content page, which needs the instanced-dialog framework (Â§0) |
| **destroy (our path)** | âœ… WORKS | `GWUI.DestroyWindow` = scrub input targets + native destroy (the safe close button, not `[X]`) |

**The single dominant open problem is the native `[X]` close crash** (dangling hover hot-item after freeing a
direct-child control). See Â§0: the real fix is the instanced-dialog + `CCtlLayout` host (which routes `[X]`
through `PopCloser`), NOT any ad-hoc container â€” all of which were tried and crash.
...
| Doc | Contents |
|---|---|
| **`native_button_pipeline.md`** | â˜… authoritative creation reference: master address/flag/status table, per-control recipes, the working-button pipeline, cross-cutting gotchas, the foundational model. |
...
| `ui_frame_system_mapping.md` | GWCA â†” WASM â†” EXE frame API mapping. |
...
- **One create primitive, one destroyer.** `FUN_0062bfc0(parent,flags,childId,proc,userdata,name)` mints a
  ...
  handle at `frame+0xbc` â€” **that returned handle is the id you keep** (no offset, no childId). Destroy =
  `FUN_0062c550(id)` â†’ `FUN_0062ab40` recursively frees the subtree AND delivers each control's own
...
- **Two-layer controls (slider, tabs) MUST be created with the WRAPPER proc as PRIMARY** via a single
  `CreateUIComponent` â€” the wrapper's case 4 installs the base. A post-create `FrameNewSubclass(wrapper)`
  never receives msg 4 and re-fires msg 9/0xb on garbage â†’ crash. (This is why the slider and tabs create
...
- **The game hosts controls under an owned content frame** (a `CtlView` scroll page, a `CtlFrameList`, or
  a plain container), never bare on the CtlDlg chrome root â€” BUT reproducing this naively regressed
  everything (see Â§6). The composite root `FUN_00876880` is window CHROME ONLY; its close handler
  `FUN_008766e0` tears down only the tab-page id band 10000..0x2718 and **does not scrub input globals**.
- **The close crash â€” attribution corrected (2026-07-01).** The engine's native teardown DOES scrub the
  frame INPUT-FOCUS globals: `FrameDestroy` (WASM `809a1b36`) â†’ `DestroyRecursive` (`8099c0ae`) calls,
  per frame, `IFrame::CMouse::Destroy(frame+0x94)` â†’ `CMouse::Reset` (`8092a9f9`), which nulls hover
  ...
  the safe-close path scrubs â€” `DAT_00c0ad54` via `FUN_00630cd0` â€” is a DIFFERENT, secondary "hot
  ...
  the destroy messages (`Notify 3`, `msg 0xb`) â€” needs one in-client confirmation rather than the
...
- **slider create crash â†’ FIXED.** Was base-primary + `FrameNewSubclass`. Now single `CreateUIComponent`
  with **`UiCtlSliderProc` 0x0087f440 as PRIMARY** (`ResolveUiCtlSliderProc`, flags 0x40000), then
  SetRange(0x56)â†’SetValue(0x57)â†’size. `CreateSliderControlByFrameId`.
- **tabs generic â†’ FIXED.** `GWUI.CreateTabs` now creates a direct-child container with **styled
  `UiCtlPageProc` 0x00885590 as PRIMARY** (`ResolveUiCtlPageProc`); `AddTab` passes tab flag 0x20000.
- **destroy silently no-ops â†’ FIXED.** GWCA `DestroyUIComponent` resolver scans an old source path
...
- **edit id=0 discarded â†’ FIXED (but still blank, see Â§5.2).** id 0 is a VALID handle (recycled slot 0);
...
- **radio rows didn't highlight â†’ row-proc corrected.** Rows now use `CtlTextSelectable` 0x00617df0
  (its case 0x57 sets the selected flag) instead of flat `CtlBtnProc` (whose 0x57 only clears a bit).
  Still broken in-client (Â§2) â€” needs more work.
- **group has no native collapse â†’ FIXED app-side.** Added `CtlFrameListShowItemByFrameId` (msg 0x67,
  by child code) + `GWUI.RegisterGroupSection` / `UpdateGroupSections`.
- **poll-after-free crashes â†’ MITIGATED.** `FrameExistsByFrameId` (GetFrameById != null); the test poll
  and `UpdateGroupSections` skip/drop controls whose window was closed.
- **safe close â†’ ADDED.** `ClearUiInputTargets()` = `FUN_00630cd0(0,-1)` + `FUN_0064e920(0)`, and
  `DestroyWindowSafelyByFrameId` scrubs then destroys. `GWUI.DestroyWindow` / harness "Destroy All" use it.
  **Fixes the close crash ONLY for our destroy path â€” NOT the native [X] (Â§5.1).**

### 5.1 OPEN â€” the close crash (dominant)
Closing a window that hosts checkbox/progress/hyperlink/radio/slider crashes via the native title-bar
**[X]**, because that runs the engine's own teardown (`FUN_008766e0` â†’ `FUN_0062c550`) which does NOT scrub
...
the window's [X] to it. See Â§6 for the approach that did NOT work.

### 5.2 OPEN â€” per-control rendering / function
- **checkbox glyph â†’ ROOT-CAUSED + FIXED (2026-07-01).** The "cancel/X button" glyph was NOT an
  image-list or handle problem â€” it was the create flags. `0x48300` includes `0x40000`, which in
  ...
  use flags `0x10000` only (auto-size-width layout) + `0x8000` (glyph) with the SAME `UiCtlBtnProc` â€” no
  `0x40000`. Fix: pass `0x10000` to `CreateCheckboxFrame` â†’ effective `0x18000`. The check glyph itself
  ...
  â€” `frame->frame_id` is the correct handle. **Close-crash is separate (Â§5.1); verify checkbox in isolation.**
- **edit**: blank. Correct outer proc (`CCtlEdit` 0x008852e0) and the id=0/seed fixes are in. The
  ...
  @ WASM `80faf332`) creates edit boxes DIRECTLY as dialog-content children â€”
  `FrameCreate(dialogContent, 0 /*or 0x1000000 password*/, childId, EditProcâ†’UiCtlEditProc(base 0xab0), 0, name)`
  plus a sibling `UiCtlBorderProc` "Border" frame â€” no special hosting container. So a blank edit is almost
...
- **slider width**: intrinsic/range-driven (measured width = `(max-min) Ã— unit Ã— scale`); there is NO
  set-width message, and the anchor-6 setter asserts on the slider. True width control needs decoupling
  the logical range from a pixel span (rescale in Get/SetSliderValue) â€” not done.

---

## 6. DEAD ENDS â€” reverted, DO NOT REPEAT
...
- **Content-panel hosting** (parent controls to a plain `FUN_0051d8e0` container instead of the window):
  intended to fix the [X] close per the model â€” **regressed EVERYTHING, even the button stopped working.**
...
- **Anchor-6 setter (0x0062F770) on the slider**: asserts inverted-rect (0x238) on the self-measuring
  slider container â†’ immediate crash. Reverted; slider uses plain `FrameSetSize` (renders wide).
...
2. **Then** revisit per-control rendering (checkbox glyph/id, edit blank, radio/hyperlink) one at a time,
   rebuilding + testing each in isolation via the harness verdict buttons â€” do NOT batch unverified changes.
3. Keep the crash-safe logging loop (`ui_test_log.txt`) â€” the `CREATING <x>` trail is how we localize hard
   crashes with no Python exception.

---

## 8. Key addresses (condensed â€” full tables in `native_button_pipeline.md` / `reverse_engineering_reference.md`)

| Symbol | EXE | Role |
|---|---|---|
| create primitive | `FUN_0062bfc0` | mint frame â†’ returns handle at `frame+0xbc` |
...
| plain container proc | `FUN_0051d8e0` | pass-through content container (dead-end host â€” Â§6) |
...
`ResolvePlainContainerProc` (dead-end, Â§6). GWUI: `CreateSlider/DestroySlider`, `CreateTabs`, `CreateEditBox`,
...
- **`Py4GW_InstancedDialogProc`** (file-scope, `__cdecl(uint32_t* msg, void*, void*)`) â€” the authored native
...
- **`ResolveFrameMsgCallBase`** â†’ `IUi_FrameMsgCallBase` EXE 0x00647170 (byte-pattern; walks the proc chain).
- **`AttachInstancedDispatcherByFrameId`** (bound `attach_instanced_dispatcher_by_frame_id`) â€” `FrameNewSubclass`
  the dispatcher onto a frame. PROVEN: engine calls it, renders, no ABI crash.
- **`CreateOwnedBandFrameByFrameId`** (bound `create_owned_band_frame_by_frame_id`) â€” band-0x2710 plain
  container + dispatcher. **Result: no contents + crash** (plain container is not a viable host â€” same dead-end
  as Â§6; kept only as scaffolding).
- **`ResolveUiCtlContentPageProc`** (composite-root âˆ’ 0x140 = EXE 0x00876740) + **`CreateContentPageByFrameId`**
  (bound `create_content_page_by_frame_id`) â€” the REAL content-page proc. **Result: crashes standalone**
  (half-initialized without the dialog framework's activation). Do not use raw.
- **`ResolveUiCtlDlgMsgProc`** (composite-root âˆ’ 0x270 = EXE 0x00876610) + **`CreateNativeDialogWindow`**
  (bound `create_native_dialog_window`) â€” window created with the dialog BASE proc (mimics
...
- **`EnsureContentFrameForWindow`** â€” DISABLED (returns window_id); the band-content nesting it drove
