...
### âœ… REGRESSION FIXED (2026-05-31)
...
1. **Primary**: `FindAssertion("UiCtlDlg.cpp", "!s_imgList", 0, 0)` â€” string-based, portable across patches
2. **Fallback**: Byte-pattern `\x81\xEC\x20\x01\x00\x00...` with 24-char mask and prologue validation

No hardcoded addresses. Runtime base differs from Ghidra static `0x00400000` â€” all resolution must use runtime-safe patterns.

**Proven resolution** (from Python diagnostic):
```
Scanner.FindAssertion("UiCtlDlg.cpp", "!s_imgList", 0, 0) â†’ 0x00EB697B
Scanner.ToFunctionStart(assertion, 0x110) â†’ 0x00EB6880 (CRProc)
```
Runtime slide: `0x00EB6880 - 0x00851180 = 0x00665600` (EXE loaded at non-standard base).

### Working (Current â€” 2026-05-31)
1. **`CreateContainerWindow()`** (existing GWCA) creates a bare `CContainerFrame` â€” works.
2. **`FrameNewSubclass(frame, Ui_CompositeRootControlProc, 0x59)`** â€” works via `FindAssertion` resolution.
3. **Mouse interaction**: `FrameMouseEnable(frame, 0xFFFFFFFF, 0)` after `FrameNewSubclass`.
4. **Python-only window creation**: Proven â€” CRProc resolved via Scanner + NativeFunction, chrome installed with `directCall()` on game thread.
...
### âš ï¸ REGRESSION (2026-05-31 â€” Old)
...
### Working (Before Regression â€” 2026-05-30)
1. **`CreateContainerWindow()`** (existing GWCA) creates a bare `CContainerFrame` with no content, no chrome â€” cold-startable with no state dependencies.
2. **`FrameNewSubclass(frame, Ui_CompositeRootControlProc, 0x59)`** (exposed via `ResolveFrameNewSubclass`) installs the window chrome â€” title bar, close button, resize handles.
...
### NOT Working (Fundamental Issue â€” RESOLVED 2026-06-02) âœ…

4. **Title/caption text**: **RESOLVED.** The root cause was non-unique byte patterns for `Ui_SetFrameText` â€” the function prologue matches 16 locations in FrApi.cpp. `Scanner::Find` returned wrong function. **Fix:** Derive `Ui_SetFrameText` from DevText's call site (find "DlgDevText" string â†’ scan forward for CALLs â†’ second CALL = `Ui_SetFrameText`). Combined with `PerFrameInvalidate(0xFFFFFFFF)`, this stores text and triggers full per-frame CContent invalidation.

**Working API**: `PyUIManager.UIManager.send_title_msg_5e(frame_id, title)` â€” delegates to `SetFrameTitleAndInvalidate()` which:
1. Calls `Ui_CreateEncodedText(8, 7, "title", 0)` â†’ encoded wchar_t*
2. Calls `Ui_SetFrameText(frame, encoded)` â†’ stores text at frame+0xCC
3. Calls `PerFrameInvalidate(frame_id, 0xFFFFFFFF)` â†’ sets all paint mask bits + dirty list enqueue
4. CRProc msg 0x08 renders the title âœ…
...
1. âœ… Text stored via `Ui_SetFrameText` â†’ attached-text table (Path B)
2. âœ… CRProc installed on frame via `FrameNewSubclass`
3. âŒ **CContent element 4 invalidation + per-frame dirty list enqueue** â€” MISSING
4. âœ… Frame shown via GWCA `ShowFrame` or native `Ui_ShowFrame`
...
| `Ui_SetFrameText` | **DevText call-site derivation** | Find "DlgDevText" string â†’ first CALL = UiCreateEncodedText â†’ second CALL = UiSetFrameText. Do NOT use byte pattern (matches 16 functions). |
| `FrameMouseEnable` | `FindAssertion` | `("FrApi.cpp", "frameId", 0x540, 0)` |
| `FrameNewSubclass` | `FindAssertion` | `("FrApi.cpp", "frameId", 0x467, 0)` |
| `CContainerFrame::FrameProc` | `FindAssertion` | `("UiPlacementContainer.cpp", "itemFrame", 0x43, 0)` |
| `PerFrameInvalidate` | `Find` | `\x8D\x48\x04\x53\x6A\x04\xE8` / `xxxxxxx` â†’ ToFunctionStart(-0x57) |

**IMPORTANT**: Byte-pattern scans on function prologues are fragile across EXE patches (e.g., stack frame size can change: `0x11C`â†’`0x120`). **Always prefer `FindAssertion` as the primary strategy**. `Ui_SetFrameText` MUST be resolved via DevText call-site derivation, NOT byte pattern.

### Known Issues

- **`[X]` close button**: Works â€” handled by CNonclient framework, not msg 0x0A.
- **Mouse interaction**: Works â€” fixed via `FrameMouseEnable(0xFFFFFFFF, 0)` after `FrameNewSubclass`.
- **Title rendering**: âœ… RESOLVED (2026-06-02) â€” via `send_title_msg_5e` â†’ `SetFrameTitleAndInvalidate`. Key fix: DevText call-site derived `Ui_SetFrameText` instead of broken byte pattern.
...
### âœ… 2026-06-03 Cleanup â€” Shared Resolver Consolidation

The C++ code in `py_ui.h` was refactored to eliminate duplicated resolution logic:

1. **`ResolveCreateEncodedText()`** â€” new shared resolver (~line 32 in `py_ui.h`, before `UIManagerTitleHook`). Single source of truth for `Ui_CreateEncodedText` pattern resolution with prologue validation. Used by `SetFrameTitleByFrameId`, `AttachCompositeRootToFrame`, and the title hook namespace. Replaces 3 previously duplicated inline scans.

2. **`ResolveSetFrameText()`** â€” new shared helper that extracts the DevText call-site derivation for `Ui_SetFrameText`. Used by both `SetFrameTitleByFrameId` and `AttachCompositeRootToFrame`. Eliminates duplicated call-site-walking logic.

3. **Static address comments removed** â€” All hardcoded-address-bearing comments (3 locations in `py_ui.h`) were removed. Function resolution is now exclusively via runtime assertion/pattern scanning.

4. **`ProcessFrameControllerUpdateByFrameId()`** â€” `AttachCompositeRootToFrame` now calls the existing `ProcessFrameControllerUpdateByFrameId` rather than resolving a raw `layout_fn` pointer via a weak byte-pattern scan.

5. **Canonical Python API** â€” `create_container_window_with_title(x, y, width, height, title)` added as a one-call titled container window creation. The older `create_window` (DevText clone path) is retained for backward compatibility.

6. **Stubs updated** â€” `stubs/PyUIManager.pyi` now includes all ~22 previously-missing C++ binding signatures.
